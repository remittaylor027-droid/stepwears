      ...
      <section style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 24px 80px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "56px", alignItems: "flex-start" }} className="pd-split">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
            {/* Category + Name */}
            <div>
              <div style={{ display: "inline-block", padding: "5px 10px", backgroundColor: "#f4dbcf", borderRadius: "6px", marginBottom: "12px" }}>
                <span style={{ fontSize: "9px", fontWeight: 800, letterSpacing: "0.18em", textTransform: "uppercase", color: "#9d5c38" }}>{product.category}</span>
              </div>
              <h1 style={{ fontSize: "clamp(1.35rem, 2.6vw, 1.875rem)", fontWeight: 800, lineHeight: 1.2, letterSpacing: "-0.02em", color: "#1a1008", margin: 0 }}>
                {product.name}
              </h1>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
              <span style={{ fontSize: "1.5rem", fontWeight: 800, color: "#b8704a", letterSpacing: "-0.02em" }}>
                Rs. {product.price.toLocaleString()}
              </span>
              {product.original_price && (
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontSize: "0.9rem", color: "#a89a92", textDecoration: "line-through", fontWeight: 600 }}>
                    Rs. {product.original_price.toLocaleString()}
                  </span>
                  {discountPct && (
                    <span style={{ fontSize: "0.72rem", fontWeight: 800, color: "#fff", backgroundColor: "#e74c3c", padding: "4px 8px", borderRadius: "6px", letterSpacing: "0.04em", display: "inline-flex", alignItems: "center" }}>
                      -{discountPct}%
                    </span>
                  )}
                </div>
            ...
            {product.colors.length > 0 && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                  <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#7a6a62", margin: 0 }}>
                    Color 
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: sizeError ? "#e74c3c" : "#7a6a62", margin: 0, display: "flex", alignItems: "center", gap: "6px" }}>
                  {sizeError ? "⚠ Select a size" : "Select Size"}
                </p>
                <Link href="/size-guide" style={{ fontSize: "0.78rem", color: "#b8704a", fontWeight: 700, textDecoration: "none", borderBottom: "1.5px dashed rgba(184, 112, 74, 0.4)", paddingBottom: "2px", transition: "border-color 0.2s" }}
                  ...
                  {product.sizes.map((s) => {
                    ...
                    return (
                      <button key={s.size} onClick={() => { if (!oos) { setSelectedSize(s.size); setSizeError(false); } }} style={{
                        ...
                        border: isSelected ? "2px solid #1a1008" : oos ? "2px solid transparent" : "1px solid #ede8e3",
            ...
            {/* Add to Cart + WhatsApp — compact scale to match PDP typography */}
            <div style={{ display: "flex", gap: "12px", marginTop: "10px", alignItems: "stretch" }}>
              <button id="pd-add-to-cart" onClick={handleAddToCart} disabled={!inStock} style={{
                flex: 1, display: "flex", alignItems: "center", justifyContent: "center",
                gap: "8px", minHeight: "48px", padding: "12px 16px", borderRadius: "12px", border: "none",
                backgroundColor: inStock ? "#1a1008" : "#d4beb2",
                color: "#fff", fontWeight: 700, fontSize: "0.875rem", letterSpacing: "0.01em",
                cursor: inStock ? "pointer" : "not-allowed",
                transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                boxShadow: inStock ? "0 6px 20px rgba(26, 16, 8, 0.2)" : "none",
                transform: "translateY(0)"
              }}
              onMouseOver={(e) => { if(inStock) { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 10px 26px rgba(26, 16, 8, 0.28)"; } }}
              onMouseOut={(e) => { if(inStock) { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(26, 16, 8, 0.2)"; } }}
              >
                <ShoppingBag style={{ width: "18px", height: "18px", flexShrink: 0 }} />
              ...
              <a href={`https://wa.me/923390912145?text=${whatsappMsg}`}
                ...
                style={{ 
                  width: "48px", minHeight: "48px", borderRadius: "12px", backgroundColor: "#25d366", 
                  display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", 
                  transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", flexShrink: 0,
                  boxShadow: "0 6px 20px rgba(37, 211, 102, 0.28)",
                  transform: "translateY(0)",
                }}
                onMouseOver={(e) => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 10px 26px rgba(37, 211, 102, 0.38)"; }}
                onMouseOut={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(37, 211, 102, 0.28)"; }}
              >
            ...
            {product.description && (
              <div style={{ marginTop: "16px" }}>
                 <p style={{ fontSize: "0.9rem", color: "#6b5a52", lineHeight: 1.75, margin: 0 }}>
                    {product.description}
                 </p>
              </div>
            )}
